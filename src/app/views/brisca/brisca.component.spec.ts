...
describe('BriscaComponent', () => {
  ...
  describe('Brisca service', () => {
    beforeEach(() => {
      ...
      gameService = TestBed.inject(GameHolderService).service as BriscaService;
      localStorage.removeItem(LOCAL_STORE_KEYS.BRISCA_LAST_TIME_DELETE_BANNER);
      localStorage.removeItem(LOCAL_STORE_KEYS.SETTINGS(gameService.gameName));
    });
    ...
    describe('Individual modality', () => {
      beforeEach(() => {
        gameService.modality = 'individual';
        gameService.playerNames = ['Player 1', 'Player 2'];
        gameService.scores = [4, 7];
      ...
      it('should allow to remove scores', () => {
        ...
        expect(gameService.scores).toEqual([3, 7]);
        expect(gameService.dealingPlayerIndex).toEqual(1);

        expect(localStorage.getItem(LOCAL_STORE_KEYS.SETTINGS(gameService.gameName))).not.toBeNull();
      });
    });
